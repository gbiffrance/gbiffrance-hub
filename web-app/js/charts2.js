...
var baseFacetChart = {
    ...
    chartLabels: {
        ...
        assertions: 'NIVEAU TAXONOMIQUE SUPERIEUR',
    ...
    title: function () { return "PAR " + this.chartLabel();},
    ...
    init: function (data, options) {
        ...
        if(options.avoidTitlePrefix !== "undefined" && options.avoidTitlePrefix){
            ...
            this.title = "Par " + this.chartLabel();
        }
...
var taxonomyChart = {
    ...
    draw: function (data) {
        ...
        opts.title = opts.name ? opts.name + " enregistré par " + rang : "PAR " + rang;
        ...
        if ($infoChart.length == 0) {
            $infoChart = $('<div class="info-chart"></div>').appendTo($centerButton);
            $infoChart.html("Cliquez sur une tranche pour accéder au niveau taxonomique inférieur.")
        }
        ...
        if ($backLink.length == 0) {
            ...
            $backLink.css('position','relative').css('top','-50px');
        ...
        if ($recordsLink.length == 0) {
            $recordsLink = $('<div class="btn btn-default access-data link button-int under last" id="recordsLink">Afficher tous les enregistrements</div>').appendTo($centerButton);  // create it
            $recordsLink.css('position','relative').css('top','-50px');
        ...
        // set link text
        if (this.hasState()) {
            $recordsLink.html('Afficher les enregistrements pour ' + this.name + '('+this.rank+')');
        }
        else {
            $recordsLink.html('Afficher tous les enregistrements');
        }
